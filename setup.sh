...
function check_inbounds_table() {
    ...
    if [[ -f "$db_path" ]]; then
        ...
        if [[ $table_exists == "inbounds" ]]; then
            read -p "The 'inbounds' table already exists. \033[32m On first run it's empty, you can safely continue. \033[0m Do you want to overwrite existing 3X-UI config? (y/n): " response
            if [[ $response == "y" ]]; then
                ./inbounds_gen.sh
